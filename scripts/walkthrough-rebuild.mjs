...
try {
  // Capture A-C: onboarding flow (fresh profile)
  const c1 = await browser.newContext({ viewport: { width: 1280, height: 1100 } })
  ...
  await p1.screenshot({ path: 'dist/rebuild-A-onboarding-welcome.png', fullPage: false })
  console.log('A: onboarding welcome')

  await p1.locator('text=Begin with the reading').click()
  await p1.waitForSelector('text=The question we start from.', { timeout: 6000 })
  await p1.waitForTimeout(700)
  await p1.screenshot({ path: 'dist/rebuild-B-brand-card-1.png', fullPage: false })
  console.log('B: brand card 1')

  await p1.locator('text=Continue reading').first().click()
  await p1.waitForSelector('text=Five things we believe.', { timeout: 6000 })
  await p1.waitForTimeout(700)
  await p1.screenshot({ path: 'dist/rebuild-C-brand-card-2.png', fullPage: false })
  console.log('C: brand card 2')
  await c1.close()

  // Capture D-F: completed profile
  const c2 = await browser.newContext({ viewport: { width: 1280, height: 1100 } })
  ...
  await p2.screenshot({ path: 'dist/rebuild-D-dashboard-today.png', fullPage: false })
  await p2.screenshot({ path: 'dist/rebuild-D-dashboard-full.png', fullPage: true })
  console.log('D: dashboard with TodayPanel')
  ...
  await p2.waitForTimeout(900)
  await p2.screenshot({ path: 'dist/rebuild-E-homes-pattern.png', fullPage: false })
  console.log('E: homes pattern section')
  ...
  await p2.screenshot({ path: 'dist/rebuild-F-trails-pattern.png', fullPage: false })
  console.log('F: trails with pattern section')
